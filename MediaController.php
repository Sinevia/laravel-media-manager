...
//        }        
    ...
    function postDirectoryCreate() {
        ...
        if ($result == true) {
            return redirect(action('MediaController@getMediaManager') . '?current_dir=' . urlencode($currentDirectory))->with('flash_success', 'Directory "' . $createDirectory . '" successfully created');
        }

        return redirect()->back()->withErrors('Creating directory "' . $createDirectory . '" failed.')->withInput();
    }

    function postDirectoryDelete() {
        $rules = array(
            //'current_dir' => 'required',
            'delete_dir' => 'required',
        );
        ...
        if ($result == true) {
            return redirect(action('MediaController@getMediaManager') . '?current_dir=' . urlencode($currentDirectory))->with('flash_success', 'Directory "' . $deleteDirectory . '" successfully deleted');
        }

        return redirect()->back()->withErrors('Deleting directory "' . $deleteDirectory . '" failed.')->withInput();
    }

    function postFileDelete() {
        $rules = array(
            //'current_dir' => 'required',
        ...
        if ($result == true) {
            return redirect(action('MediaController@getMediaManager') . '?current_dir=' . urlencode($currentDirectory))->with('flash_success', 'File "' . $deleteFile . '" successfully deleted');
        }
    ...
    function postFileUpload() {
        ...
        try {
            ...
            $redirectUrl = action('MediaController@getMediaManager') . '?current_dir=' . urlencode($currentDirectory);
